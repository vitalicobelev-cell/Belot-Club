...
const defaultSettings = {
    soundEnabled: true,
    theme: "blue" // "blue" | "graphite" | "black"
};
...
function applyTheme(theme) {
    ...
    // Обновляем превью в настройках (если открыто)
...
function fillRules() {
    ...
    rules.innerHTML = `
        <h3>🎯 Цель игры</h3>
        <p>
            Побеждает команда (или игрок в режиме 3), первой набравшая <b>101</b> очко или больше.<br>
        ...
        <p>
            Базовая стоимость игры — <b>16</b> очков.<br>
            Каждое объявление добавляет свои очки к стоимости.<br>
            <b>Пример:</b> 16 + Тэрц (+2) + Бэла (+2) = <b>20</b>
        </p>

        <h3>📣 Объявления</h3>
        <p>
            Объявления можно делать <b>до</b> начала розыгрыша.<br>
            Они влияют на стоимость игры и записываются в историю.
        </p>
        ...
        <table class="rulesTable">
            ...
            <tr><td><b>50</b></td><td>+4</td><td>✅ Да</td><td>50 очков за взятки</td></tr>
            <tr><td><b>100</b></td><td>+8</td><td>✅ Да</td><td>100 очков за взятки</td></tr>
        ...
        <h3>⚡ Болт</h3>
        <p>
            Если играющий набрал <b>меньше половины</b> стоимости игры, но <b>больше нуля</b> —<br>
            он получает болт, а все очки игры переходят соперникам.
        </p>
        <p>
            <b>3 болта → −10 очков</b>, после чего болты обнуляются.
        </p>

        <h3>🚫 Нет взятки (Капот)</h3>
        <p>
            Если соперники набрали <b>0</b> очков — они получают штраф <b>-10</b>,<br>
            а играющий получает все очки игры.
        </p>

        <h3>❌ Неправильная раздача</h3>
        <p>
            При результате <b>0 : 0</b> (все игроки набрали 0) —<br>
            раздающий получает <b>-10</b>, очки игры никому не начисляются.
        </p>

        <h3>👑 Кто играет</h3>
        <p>
            В режиме <b>4 игрока</b>: играет один игрок, его партнёр — в той же команде.<br>
            В режиме <b>3 игрока</b>: играет один игрок, соперники — двое других.
        </p>
        <p>
            Играющий <b>обязан</b> набрать хотя бы 2 очка (если соперники не набрали 0).
        </p>
    `;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Обработчик для activePlayerSelect (4 игрока)
    ...
    // Обработчик для activePlayerSelect3
...
function unlockAudio() {
    try {
        ...
        // Короткий тихий сигнал для разблокировки
...
// Основная функция воспроизведения тона
...
// Мелодия для клика (короткий "тик")
function playClick() {
    playTone(1200, 0.04, "square", 0.05);
}

// Мелодия для OK (приятный аккорд)
function playOk() {
...
// Мелодия для штрафа (печальный "ду-ду")
...
// Мелодия для победы (мажорный арпеджио)
function playWin() {
...
let pendingTheme = settings.theme; // Временная тема

function openSettings() {
    pendingTheme = settings.theme; // Сбрасываем при открытии
    updateSettingsUI();
...
function closeSettings() {
    ...
    // Возвращаем старую тему при отмене
    applyTheme(settings.theme);
    updateSettingsUI();
}
...
function setTheme(theme) {
    // Прямая установка темы (для совместимости со старым кодом)
    pendingTheme = theme;
...
// Сохраняем настройки при загрузке
document.addEventListener("DOMContentLoaded", () => {
    // ... существующий код ...
    updateSettingsUI();
});
